...
def fetch_quotes(symbol_map: dict[str, str]) -> list[dict]:
    """Return [{name, symbol, last, pct_change}] using yfinance."""
    rows = []
    if not HAVE_YF:
        return rows
    for name, sym in symbol_map.items():
        ...
        try:
            t = yf.Ticker(sym)
            last = prev = opn = hi = lo = None
            # fast_info is the freshest intraday source (updates through the session)
            try:
                fi = t.fast_info
                last = _fi_get(fi, "last_price", "lastPrice")
                prev = _fi_get(fi, "previous_close", "previousClose")
                opn  = _fi_get(fi, "open")
                hi   = _fi_get(fi, "day_high", "dayHigh")
                lo   = _fi_get(fi, "day_low", "dayLow")
            except Exception:
                pass
            # fall back to daily bars if fast_info is unavailable
            if last is None or prev is None:
                hist = t.history(period="7d")
                closes = hist["Close"].dropna()
                if len(closes) >= 2:
                    last, prev = float(closes.iloc[-1]), float(closes.iloc[-2])
                    row["asof"] = closes.index[-1].date().isoformat()
                    try:
                        opn = float(hist.loc[closes.index[-1], "Open"])
                    except Exception:
                        pass
            ...
            if last is not None and prev not in (None, 0) and not (
                ...
                # SANITY: a valid live price must sit inside today's [low, high].
                # If it doesn't, the feed handed us a stale/bad bar → flag it.
...
def fetch_fii_dii() -> list[dict]:
    """
    Daily FII/DII cash-market provisional figures from NSE with Moneycontrol fallback.
    """
    out = []
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
    ...
    # Fallback: Moneycontrol public API
    try:
        mc_url = "https://www.moneycontrol.com/mc/widget/fii_dii/get_table_data"
        mc_headers = {"User-Agent": "Mozilla/5.0"}
        res = requests.get(mc_url, headers=mc_headers, timeout=5)
        if res.status_code == 200:
            data = res.json()
            # Parse FII and DII rows
            fii_net = data.get("fii_net", "0")
            dii_net = data.get("dii_net", "0")
            dt_str = data.get("date", "")
            out = [
                {"category": "FII/FPI *", "date": dt_str, "buy": "-", "sell": "-", "net": str(fii_net)},
                {"category": "DII **", "date": dt_str, "buy": "-", "sell": "-", "net": str(dii_net)}
            ]
            return out
    except Exception:
        pass
...
SECTOR_FACTORS = {
    "Banks/Financials": {
        # SIGN CORRECTION: oil and us10y previously carried +0.02 / +0.05 here, the
        # OPPOSITE sign to the calibrated Bank Nifty betas (-0.04 / -0.09 in
        # SENSITIVITY) and to our own transmission map (oil↑ → inflation → RBI →
        # 🔴 banks). A judgement prior must never silently override a fitted value.
        # Deferring to calibration; the Phase-1 NIM tailwind lives in india_cpi_hot,
        # which is a slower, separate channel from a same-day oil/yield move.
        "india_cpi_hot": +0.10,   # PHASE 1: higher rates → NIM tailwind (banks + early)
        "us10y_pct":     -0.06,   # was +0.05 — calibrated Bank Nifty is -0.09
        "fii_kcr":       +0.06,   # heavy FII ownership — flow sensitive
        "oil_pct":       -0.03,   # was +0.02 — oil↑ ⇒ inflation ⇒ yields ⇒ MTM/growth hit
        "vix_pct":       -0.05,   # high-beta — risk-off hurts
    },
    "IT services": {
        "usdinr_pct":     +0.10,  # weaker rupee lifts export earnings
        # SOX is a MEDIUM, REGIME-DEPENDENT driver — not a major one, and its sign
        # FLIPS (see REGIME_FLIP). Under Substitution, a chip rally is capital
        # rotating INTO AI infrastructure and OUT of services: a headwind for Indian
        # IT, not a tailwind. Same logic AI_REGIME_LINKS already applies in the
        # scorecard; the sector model was contradicting it with a flat +0.04.
        "sox_pct":        +0.03,  # was +0.04 major-positive; now medium & sign-flipping
        "ai_substitution": -0.25, # was -0.30; trimmed — overlaps the SOX flip below
        "us_cpi_cool":    +0.06,  # cooler US CPI → softer landing → client IT spend
        "fii_kcr":        +0.05,
    },
    # AUTO IS NOT ONE SECTOR ON OIL. Treating it as a monolith with an ICE-weighted
    # oil coefficient contradicted our own transmission map ("costly petrol makes EVs
    # relatively cheaper to run") and NewsAgent's sub-sector model, which already has
    # oil at -0.30 for PV but +0.30 for EV. A single sector score cannot express a
    # relationship whose SIGN flips within the sector, so it is split.
    "Auto — ICE (PV/2W)": {
        "oil_pct":       -0.05,   # fuel cost → running cost → demand
        "india_cpi_hot": -0.08,   # rate-sensitive discretionary / financed purchase
        "us10y_pct":     -0.03,
        "fii_kcr":       +0.04,
    },
    "Auto — EV": {
        "oil_pct":       +0.04,   # SIGN FLIPS: dear petrol improves EV running economics
        "ev_theme":      +0.10,   # policy/PLI/launch news — the dominant EV driver
        "india_cpi_hot": -0.06,   # still discretionary & financed, so rates still bite
        "copper_pct":    -0.03,   # battery / wiring input cost
        "fii_kcr":       +0.04,
    },
    "Pharma": {
    ...
    "Metals": {
        # KOSPI REMOVED. The Kospi is ~40% Samsung + SK Hynix — a semiconductor index
        # wearing a country's name. Its link to Indian metals is proxy-by-accident
        # (Korean steel supply is real but third-order). Replaced with COPPER, the
        # direct industrial-metals demand read we already fetch (HG=F) and never used.
        "copper_pct": +0.12,      # 'Dr Copper' — the actual growth/demand barometer
        "dxy_pct":    -0.10,      # stronger dollar → commodity headwind
        "oil_pct":    +0.03,      # reflation read-through (NOT level-amplified — see
                                  # OIL_LEVEL_SECTORS: for metals oil is an input cost)
        "fii_kcr":    +0.04,
    },
    "Energy (upstream)": {
...
# ── CORRELATED-FACTOR BLOCKS ───────────────────────────────────────────────
# Credit growth, deposit growth, the credit-deposit gap and bank earnings are NOT
# independent signals. Bank PAT ≈ (credit growth × NIM) − credit costs, so earnings
# are largely the REALISED OUTCOME of credit growth, not a separate confirming fact.
# Adding them as separate additive terms counts one economic phenomenon two or three
# times and calls it corroboration.
#
# Fix: correlated factors share ONE weight budget. Internal weights sum to 1.0 and are
# scaled by the block budget — so adding a factor RE-SLICES the pie, it never grows it.
#
# Missing data SHRINKS the block (budget × coverage) instead of renormalising onto the
# survivors. That's deliberate: the old behaviour let inactive positive fundamentals
# hand their weight to the surviving negative macro factors, making Banks bearish by
# construction on any rate-stress day.
FUNDAMENTAL_BUDGET = {"Banks/Financials": 0.25}
FUND_BLOCK = {
    "Banks/Financials": {
        # sub-factor: (internal weight, sign)
        "credit_growth":  (0.40, +1),   # volume / earning assets
        "deposit_growth": (0.20, +1),   # cheap stable funding
        "cd_gap":         (0.25, -1),   # credit outrunning deposits → funding cost ↑ → NIM ↓
        "earnings":       (0.15, +1),   # realised confirmation, deliberately the SMALLEST
    },
}
...
def _num0(v):
    """None/NaN -> 0.0 (NaN is truthy, so `or 0.0` is not enough)."""
    return 0.0 if v is None or (isinstance(v, float) and math.isnan(v)) else float(v)


def _fundamental_block(sector, earnings_signal):
    """Return (contribution, coverage, detail_rows) for a sector's correlated block."""
    spec = FUND_BLOCK.get(sector)
    if not spec:
        return 0.0, None, []
    sig = {}
    try:
        import rbi_credit
        sig = (rbi_credit.get() or {}).get("signals", {}) or {}
    except Exception:
        sig = {}
    if earnings_signal is not None:
        sig["earnings"] = earnings_signal

    budget = FUNDAMENTAL_BUDGET.get(sector, 0.25)
    tot_w = sum(w for w, _ in spec.values())
    got_w, raw, rows = 0.0, 0.0, []
    for key, (w, sgn) in spec.items():
        ...
        got_w += w
        raw += w * sgn * v
        rows.append((key, round(budget * w * sgn * v, 3)))
    if got_w <= 0:
        return 0.0, 0.0, []
    # coverage scaling — missing data shrinks influence, it does NOT amplify survivors
    contrib = budget * (raw / tot_w)
    return round(contrib, 3), round(got_w / tot_w, 2), rows


# (sector, driver) pairs whose SIGN flips under the AI-Substitution regime. Mirrors
# AI_REGIME_LINKS in the scorecard so the two never disagree about the same day.
REGIME_FLIP = {("IT services", "sox_pct")}

# Oil's LEVEL multiplier models the MACRO channel (CAD → inflation → rupee → rates).
# It should only amplify sectors that feel oil macro-economically. For Metals, oil is
# a direct input cost (smelting), so multiplying a reflation prior by the macro
# multiplier over-states it — that artifact alone flipped Metals to Bullish once.
OIL_LEVEL_SECTORS = {"Banks/Financials", "Auto — ICE (PV/2W)", "Energy (upstream)"}
# NOTE: "Auto — EV" is deliberately NOT here. The level multiplier models oil's MACRO
# stress channel; for EV the oil link is a relative-running-cost nudge, and amplifying
# it ×1.4 at $88 would overstate a second-order substitution effect.

# The Substitution thesis reaches IT through TWO correlated channels (the regime flag
# and the SOX sign-flip). Left unbounded that double-counts one story — the same
# correlated-evidence error the fundamentals block was built to avoid. Cap the pair.
AI_THESIS_CAP = 0.35
...
_SECTOR_KW = {
    ...
    "Auto — ICE (PV/2W)": ["auto", "maruti", "hero", "bajaj", "tvs", "eicher", "passenger vehicle"],
    "Auto — EV": ["ev ", "electric vehicle", "e-scooter", "ather", "ola electric",
                  "tata motors", "mahindra", "battery"],
...
def build_sector_factor_model(eng, quotes_macro, observed, news, ai_regime):
    ...
    vals = {
        ...
        # Dr Copper — the industrial-metals demand read (replaces Kospi for Metals)
        "copper_pct":    _num0(_pct_of(quotes_macro, "Copper")),
    }
    ...
    # ---- AUDIT: record inputs with provenance, distinguishing measured from assumed ----
    trail = None
    try:
        from audit import AuditTrail
        trail = AuditTrail(engine="sector_factor_model")
        _src = {"oil_pct": "yfinance BZ=F", "us10y_pct": "yfinance ^TNX",
                "dxy_pct": "yfinance DX-Y.NYB", "kospi_pct": "yfinance ^KS11",
                "sox_pct": "yfinance ^SOX", "vix_pct": "yfinance ^INDIAVIX",
                "fii_kcr": "NSE FII/DII", "usdinr_pct": "yfinance USDINR=X"}
        for k, v in vals.items():
            if k in ("india_cpi_hot", "us_cpi_cool", "ai_substitution", "ev_theme"):
                trail.input(k, v, source="news keyword flag", kind="flag",
                            note="binary detection — presence, not magnitude")
            else:
                trail.input(k, v, source=_src.get(k, "market data"), kind="observed")
        try:                                   # RBI fundamentals, if present
            import rbi_credit
            _r = rbi_credit.get() or {}
            for k, v in (_r.get("signals") or {}).items():
                trail.input(k, v, source=_r.get("source") or "RBI WSS",
                            asof=_r.get("as_of") or "", stale=_r.get("stale", False),
                            kind="fitted", note="normalised from RBI y/y growth")
        except Exception:
            pass
    except Exception:
        trail = None
    ...
    # The sector model previously read RAW driver values — bypassing every safeguard
    # the causal engine applies. That let a −6.4% Kospi print land un-capped and
    # single-handedly decide Metals, which is the exact failure the factor-library
    # design set out to prevent ("Kospi drives Metals — economically wrong").
    oil_mult = eng.get("oil_mult") or _oil_level_mult(eng.get("brent_price"))
    ...
    for sector, coefs in SECTOR_FACTORS.items():
        ...
        for k, c in coefs.items():
            v = vals.get(k, 0.0)
            # ① outlier cap — same DRIVER_CAPS the causal engine uses
            cap = DRIVER_CAPS.get(k)
            is_capped = cap is not None and abs(v) > cap
            v_used = (cap * (1 if v > 0 else -1)) if is_capped else v
            # ② oil is NON-LINEAR in its LEVEL — but only through the MACRO channel,
            #    so amplify solely for sectors in OIL_LEVEL_SECTORS.
            lvl = (oil_mult if (k == "oil_pct" and sector in OIL_LEVEL_SECTORS) else 1.0)
            # ③ regime-dependent SIGN: SOX→IT inverts under AI-Substitution
            flip = -1 if ((sector, k) in REGIME_FLIP and ai_regime == "Substitution") else 1
            contrib = c * v_used * lvl * flip
            contribs[k] = contrib
            if abs(contrib) < 0.005:      # skip inactive drivers
                continue
            note = "coefficient is a judgement prior, not a fitted beta"
            if is_capped:
                note += f" · ⚠ driver capped {v:+.2f}→{v_used:+.2f}"
            if lvl != 1.0:
                note += f" · oil level ×{lvl:.1f} (Brent ${eng.get('brent_price') or 0:.0f})"
            if flip < 0:
                note += " · ⚡ SIGN FLIPPED by AI-Substitution regime"
            rows.append((lbl.get(k, k) + (" ⚠cap" if is_capped else ""), round(contrib, 3)))
            net += contrib
            if trail:
                ops = {"driver": round(v, 3), "coef": c}
                if is_capped:
                    ops["capped_to"] = v_used
                if lvl != 1.0:
                    ops["oil_level_mult"] = round(lvl, 2)
                trail.step(sector, lbl.get(k, k), round(contrib, 3),
                           formula="driver_move × sensitivity × level_mult",
                           operands=ops, kind="prior", note=note)
            if is_capped and trail:
                trail.caveat(sector, f"{lbl.get(k,k)} moved {v:+.2f} — an outlier; capped to "
                                     f"{v_used:+.2f} so one print cannot decide the sector", "warn")

        # AI-thesis cap: the regime flag and the SOX sign-flip express ONE story. Bound
        # their combined pull so a single thesis cannot run away with the sector.
        if sector == "IT services":
            ai_pair = contribs.get("ai_substitution", 0.0) + contribs.get("sox_pct", 0.0)
            if ai_pair < -AI_THESIS_CAP:
                excess = ai_pair + AI_THESIS_CAP          # negative
                net -= excess
                contribs["ai_substitution"] -= excess
                rows = [("AI thesis ⚠capped", round(-AI_THESIS_CAP, 3))
                        if r[0] in ("AI substitution", "SOX") else r for r in rows]
                if trail:
                    trail.step(sector, "AI thesis (combined cap)", round(-AI_THESIS_CAP, 3),
                               formula="cap(ai_substitution + sox_pct)",
                               operands={"before": round(ai_pair, 3), "cap": -AI_THESIS_CAP},
                               kind="prior",
                               note="regime flag and SOX flip are the SAME thesis — bounded "
                                    "so correlated evidence isn't double-counted")
                    trail.caveat(sector, "AI-substitution reaches IT via two correlated "
                                         "channels; combined pull capped", "warn")

        # ③ weak-transmission dominance cap — Kospi/SOX reach Indian sectors only via a
        # long chain, so they may not exceed MAX_WEAK_DOMINANCE of the sector's force.
        for wk in WEAK_TRANSMISSION:
            if abs(contribs.get(wk, 0.0)) < 0.005:
                continue
            others = sum(abs(x) for kk, x in contribs.items() if kk != wk)
            allowed = MAX_WEAK_DOMINANCE / (1 - MAX_WEAK_DOMINANCE) * others
            if abs(contribs[wk]) > allowed:
                before = contribs[wk]
                after = allowed * (1 if before > 0 else -1)
                net += (after - before)
                contribs[wk] = after
                lw = lbl.get(wk, wk)
                rows = [(f"{lw} ⚠capped", round(after, 3)) if r[0].startswith(lw) else r
                        for r in rows]
                if trail:
                    trail.step(sector, f"{lw} (dominance cap)", round(after, 3),
                               formula="cap = MAX/(1-MAX) × Σ|other contributions|",
                               operands={"before": round(before, 3),
                                         "MAX": MAX_WEAK_DOMINANCE},
                               kind="prior",
                               note="weak transmission — cannot dominate a sector")
                    trail.caveat(sector, f"{lw} contribution cut {before:+.3f}→{after:+.3f}: it "
                                         f"reaches Indian sectors only indirectly and was "
                                         f"exceeding {int(MAX_WEAK_DOMINANCE*100)}% of the total",
                                 "warn")
        # Earnings kicker — DIRECTIONAL.
        # BUGFIX: this used to add +0.05 for ANY sector-tagged results headline, so a
        # bank reporting a profit COLLAPSE scored identically to one reporting a beat.
        # That hardcoded "earnings season = bullish". Sign it from beat/miss language
        # (reusing POS_WORDS/NEG_WORDS — single source of truth, per repo DRY rule).
        ek, n_pos, n_neg = 0.0, 0, 0
        for n in news:
            t = (n.get("title", "") + " " + n.get("tags", "")).lower()
            if not (_kw_match(t, ["result", "results", "earnings", "q1", "q2", "q3", "q4",
                                  "profit", "pat"])
                    and _kw_match(t, _SECTOR_KW.get(sector, []))):
                continue
            if _kw_match(t, ["beat", "beats", "tops", "record profit", "above estimate"]):
                sgn = +1
            elif _kw_match(t, ["miss", "misses", "below estimate", "shortfall", "provision"]):
                sgn = -1
            else:                                   # fall back to tone words
                p = sum(1 for w in POS_WORDS if w in t)
                q_ = sum(1 for w in NEG_WORDS if w in t)
                sgn = 1 if p > q_ else -1 if q_ > p else 0
            ek += 0.05 * sgn
            n_pos += sgn > 0
            n_neg += sgn < 0
        ek = max(-0.15, min(0.15, ek))              # symmetric cap

        if sector in FUND_BLOCK:
            # Correlated block: earnings folds IN (shares the budget) rather than
            # stacking on top of credit/deposit growth — see FUND_BLOCK note.
            e_sig = _clampf(ek / 0.15) if (n_pos or n_neg) else None
            fc, cov, frows = _fundamental_block(sector, e_sig)
            if frows:
                rows += [(f"{k} (fund.)", v) for k, v in frows]
                net += fc
                if trail:
                    for k, v in frows:
                        w, sgn = FUND_BLOCK[sector][k]
                        trail.step(sector, f"{k} (fund.)", v,
                                   formula="budget × internal_w × sign × signal",
                                   operands={"budget": FUNDAMENTAL_BUDGET.get(sector, 0.25),
                                             "w": w, "sign": sgn},
                                   kind="fitted" if k != "earnings" else "observed",
                                   note="correlated block — shares one budget, does not stack")
                if cov is not None and cov < 0.6:
                    rows.append((f"⚠ fundamentals {int(cov*100)}% covered", 0.0))
                    if trail:
                        trail.caveat(sector,
                                     f"fundamentals only {int(cov*100)}% covered — block scaled "
                                     f"DOWN by coverage (missing data reduces confidence; it is "
                                     f"NOT renormalised onto surviving factors)", "warn")
        elif ek:
            rows.append((f"Earnings ({n_pos}+/{n_neg}−)", round(ek, 3)))
            net += ek
            if trail:
                trail.step(sector, f"Earnings ({n_pos}+/{n_neg}−)", round(ek, 3),
                           formula="Σ 0.05 × sign(beat/miss), capped ±0.15",
                           operands={"positive": n_pos, "negative": n_neg},
                           kind="observed", note="directional since bugfix")
        rows.sort(key=lambda r: -abs(r[1]))
        verdict = ("🟢 Bullish" if net > 0.10 else "🔴 Bearish" if net < -0.10 else "🟡 Neutral")
        if trail:
            trail.decision(sector, verdict, round(net, 3),
                           {"bullish": 0.10, "bearish": -0.10})
        out.append({"sector": sector, "rows": rows, "net": round(net, 3), "verdict": verdict})
    out.sort(key=lambda s: -s["net"])
    if trail:
        out_audit = {"_audit": trail}
        for s in out:
            s["_trail"] = trail
        globals()["_LAST_TRAIL"] = trail
    return out
...
def _level_of(quotes, name):
    """PRICE LEVEL with a fallback, for level-dependent logic (oil bands, amplifiers).

    `last` and `pct_change` are populated by different code paths, so a quote can carry
    a valid % move while `last` is None. Everything level-based then silently went
    dormant — the Brent band table, the ×1.4 amplifier and OIL_LEVEL_SECTORS all
    disappeared while the % move kept flowing, and the report only said "n/a".
    Falling back to previous_close keeps the LEVEL logic alive (a level is a slow
    variable; yesterday's close is a fine proxy for which $10 band we are in).
...
def _oil_level_mult(price) -> float:
    """Level-dependent amplifier for oil's market impact (psychological barriers).
    Same % move bites harder the higher the absolute Brent price."""
    if price is None:
        return 1.0
    if price < 75:  return 0.6    # cheap oil — muted impact
    if price < 85:  return 1.0    # normal
    if price < 95:  return 1.4    # approaching/above $90 — stress
    if price < 110: return 1.8    # crisis zone
    return 2.2                     # shock
...
RELATIONSHIPS = [
    ...
    ("Copper → base-metal producers up", "copper", 0.7,
        [("Tata Steel", +1)]),
    # SPLIT: IT services and EMS are siblings with OPPOSITE regime behaviour (see
    # market_engine.py note). EMS lives only in extra_validations now.
...
ECON_RATIONALE = {
    ...
    "US semis (SOX) → Indian IT services": 3,      # PROXY, not supply chain
    "Kospi (AI proxy) → Indian IT": 1,             # incidental correlation, NOT transmission
...
def enrich_earnings(earnings: list[dict]) -> list[dict]:
    ...
    for e in candidates:
        ...
        if sym in seen:
            continue
        if enriched >= MAX_EARNINGS_ENRICH:
            break
        seen.add(sym)
        try:
