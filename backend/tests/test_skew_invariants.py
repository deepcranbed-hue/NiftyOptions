...
from backend.quant.skew.skew_engine import (_b76_call, decompose_skew, iv_at_delta, build_otm_smile,
                         forward_from_parity, parity_gate, flow_join,
                         classify_configuration, z_skew, PRIOR)
from backend.quant.skew import invariants
...
def test_25_interpolated_rr_matches_analytic():
    ...
    from backend.quant.skew.skew_engine import delta
...
def test_23g_unclassified_and_no_nearest_match():
    ...
    import inspect
    from backend.quant.skew import skew_engine
    src = inspect.getsource(skew_engine.classify_configuration)
    assert "nearest" not in src.lower()                            # structural absence

# ----------------------------- test 26: DTE splice ----------------------------------
def test_26_dte_two_tier():
    """D-MA-12 (supersedes binary gate): hard floor gaps; caution band computes with EXPIRY_REGIME flag;
    splice preferred when next chain exists."""
    ch = make_chain(24300, T3, SKEWED(24300), STRIKES)
    # below hard floor -> gap
    out0 = decompose_skew(ch, ch, T3, T3, dte_days=0.1, spot_open=24300, spot_curr=24300)
    assert out0["status"] == "EXPIRY_DEGENERATE"
    # caution band (hard floor <= dte < splice) -> computed + flagged
    out1 = decompose_skew(ch, ch, T3, T3, dte_days=1.0, spot_open=24300, spot_curr=24300)
    assert out1["status"] in ("OK", "PARTIAL") and "EXPIRY_REGIME" in out1["flags"]
    # full splice (both snapshots) -> used, SPLICED flag, no regime flag
    nxt_o = make_chain(24300, 8/365, SKEWED(24300), STRIKES)
    nxt_c = make_chain(24310, 8/365, SKEWED(24300), STRIKES)
    out2 = decompose_skew(ch, ch, 8/365, 8/365, dte_days=1.0,
                          spot_open=24300, spot_curr=24310,
                          next_expiry_open_chain=nxt_o, next_expiry_curr_chain=nxt_c)
    assert out2["status"] in ("OK", "PARTIAL")
    assert "SPLICED" in out2["flags"] and "EXPIRY_REGIME" not in out2["flags"]
    # half-splice (current only) -> refused (D-MA-14)
    out3 = decompose_skew(ch, ch, T3, T3, dte_days=1.0,
                          spot_open=24300, spot_curr=24300, next_expiry_curr_chain=nxt_c)
    assert out3["status"] == "SPLICE_INCOMPLETE"
...
def test_23h_extreme_share_state():
    """Rolling masks repricing: big fixed move, near-zero floating move -> EXTREME, not a headline ratio."""
    F0 = 24300
    # kinked smile whose whole level shifts with a slope tweak such that the
    # floating RR (constant relative distance) barely moves while fixed strikes reprice
    base = lambda K: 0.12 + 0.00004 * max(F0 - K, 0) - 0.00001 * max(K - F0, 0)
    open_ch = make_chain(F0, T3, base, STRIKES)
    F1 = F0 * 1.006
    shifted = lambda K: 0.12 + 0.00004 * max(F1 - K, 0) - 0.00001 * max(K - F1, 0)  # smile re-anchored to new spot
    curr_ch = make_chain(F1, T3, shifted, STRIKES)
    out = decompose_skew(open_ch, curr_ch, T3, T3, 3, F0, F1)
    a = out["artifact_share"]
    if a["status"] == "EXTREME":
        assert "value" not in a and "value_raw" in a          # share demoted, never headline
        assert "d_rr_fixed_vpt" in a and "d_rr_floating_vpt" in a
    else:
        # if this fixture lands in QUIET/OK on some grid, the state machine must still be one of the honest states
        assert a["status"] in ("QUIET", "OK", "MIXED_REGIME")


def test_23i_flow_on_oi_without_spreads():
    """D-MA-11: LTP-era chains (no bid/ask) still produce a flow block on OI, flagged NO_SPREAD_DATA."""
    j = flow_join(d_iv_vpt=-1.8, d_oi_pct=+35.0, d_spread_pct=None)
    assert j["state"] == "SUPPLY_OR_UNWIND"
    assert "NO_SPREAD_DATA" in j["flags"] and "SPREAD_WIDENED" not in j["flags"]


def test_23j_vocabulary_covers_emitted_codes():
    """Every code the engine can emit has a one-line meaning in VOCABULARY."""
    from backend.quant.skew.skew_engine import VOCABULARY
    ch = make_chain(24300, T3, SKEWED(24300), STRIKES)
    F1 = 24300 * 1.004
    curr = make_chain(F1, T3, lambda K: SKEWED(24300)(K) + 0.004, STRIKES)
    out = decompose_skew(ch, curr, T3, T3, dte_days=1.0, spot_open=24300, spot_curr=F1)
    assert out["status"] in VOCABULARY["status"] or out["status"] == "EXPIRY_DEGENERATE"
    for f in out["flags"]:
        assert f in VOCABULARY["flags"], f
    for nm in ("call25_open", "put25_open", "call25_curr", "put25_curr"):
        assert out[nm]["status"] in VOCABULARY["wing.status"]
    assert out["artifact_share"]["status"] in VOCABULARY["artifact_share.status"]
    if isinstance(out.get("flow"), dict) and "state" in out["flow"]:
        assert out["flow"]["state"] in VOCABULARY["flow.state"]
        for f in out["flow"]["flags"]:
            assert f in VOCABULARY["flow.flags"]


def test_23k_ltp_tier_parity_tolerance():
    """D-MA-13: strikes without quotes use the LTP-tier tolerance (labeled), not the live 0.5;
    genuinely broken strikes (gap >= 5 vpt) are still excluded."""
    from backend.quant.skew.skew_engine import parity_gate, forward_from_parity
    F0 = 24300
    ch = make_chain(F0, T3, SKEWED(F0), STRIKES)
    ch = ch.drop(columns=["bid", "ask"])              # LTP-era: no quotes
    # pollute one strike's PE mid to create a ~large parity gap
    k_bad = 24800
    ch.loc[(ch.strike == k_bad) & (ch.cp == "PE"), "mid"] *= 1.6
    clean, flagged = parity_gate(ch, forward_from_parity(ch), T3)   # median forward robust to one bad strike
    reasons = {f["strike"]: f for f in flagged if "tol_source" in f}
    # every flagged tolerance is labeled ltp_tier (no quotes anywhere)
    assert all(f["tol_source"] == "PRIOR:ltp_tier" for f in reasons.values())
    # mild sync-noise strikes are NOT excluded at 3.0 (most of the grid is clean)
    assert len(clean) > len(STRIKES) * 0.7


def test_23l_tb_epsilon_matches_guard():
    """Guard/checker sync (08-Jul false alarm): near-zero fixed leg with opposite-sign
    floating is the pure sticky-strike case — T-B must PASS on the OK share the guard emits."""
    emission = {
        "rr_floating": {"d_vpt": -0.381, "window": "w"},
        "rr_fixed": {"d_vpt": 0.013, "window": "w"},
        "legs_fixed_vpt": {"d_call": 0.010, "d_put": -0.003},
        "artifact_share": {"status": "OK", "value": 1.034},
        "parity_flags": {"open": [], "current": []},
    }
    inv = invariants.evaluate(emission)
    tb = next(r for r in
              ([f for f in inv["failures"]] + [{"id": c, "result": "PASSED"} for c in inv["checked"]])
              if r["id"] == "T-B")
    assert tb.get("result", "FAILED" if tb in inv["failures"] else "PASSED") != "FAILED" or tb not in inv["failures"]
    assert not any(f["id"] == "T-B" for f in inv["failures"])
    # and a REAL sign conflict (fixed leg well above tolerance) still fails without MIXED_REGIME
    emission2 = dict(emission)
    emission2["rr_fixed"] = {"d_vpt": 0.90, "window": "w"}
    emission2["legs_fixed_vpt"] = {"d_call": 0.90, "d_put": 0.0}
    inv2 = invariants.evaluate(emission2)
    assert any(f["id"] == "T-B" for f in inv2["failures"])
