...
def index_bias(sector_sentiment_stats: dict[str, dict], sw: dict[str, float] | None = None) -> tuple[float, float, dict]:
    ...
    # 1. No-news rule: renormalize over covered sectors (ignore __drilldown)
    covered_sectors = [s for s in sector_sentiment_stats if not s.startswith("__") and s in sw and (sector_sentiment_stats[s]["combined"] != 0.0 or sector_sentiment_stats[s]["direct_n"] > 0 or sector_sentiment_stats[s]["derived_n"] > 0)]
    
    total_articles = sum(v["direct_n"] + v["derived_n"] for k, v in sector_sentiment_stats.items() if not k.startswith("__"))
    ...
    stats = {
        ...
        "direct_n": sum(v["direct_n"] for k, v in sector_sentiment_stats.items() if not k.startswith("__")),
        "derived_n": sum(v["derived_n"] for k, v in sector_sentiment_stats.items() if not k.startswith("__")),
    }
