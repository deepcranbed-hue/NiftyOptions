...
def parity_gate(chain: pd.DataFrame, F: float, T: float,
    ...
    for K in sorted(c.index.intersection(p.index)):
        ...
        tol_used, tol_src = (tol_k, "DERIVED:spread") if tol_k is not None else (tol_vpt, "PRIOR:fallback")
        if gap_vpt > tol_used:
...
# ----------------------------- flow join (D-MA-08) ----------------------------------
def flow_join(d_iv_vpt: float, d_oi_pct: float, d_spread_pct: float,
    ...
    flags = []
    if d_spread_pct > spread_widen_pct:
...
def leg_flow_inputs(open_chain: pd.DataFrame, curr_chain: pd.DataFrame,
    ...
    """
    need = {"bid", "ask", "oi"}
    if not need.issubset(open_chain.columns) or not need.issubset(curr_chain.columns):
        return None
    ...
    if sp_o <= 0 or oi_o <= 0:
        return None
    return {"strikes": [int(k) for k in Ks], "cp": cp,
            "d_spread_pct": (sp_c / sp_o - 1.0) * 100.0,
            "d_oi_pct": (oi_c / oi_o - 1.0) * 100.0,
            "spread_open": sp_o, "spread_curr": sp_c}
...
def decompose_skew(open_chain: pd.DataFrame, curr_chain: pd.DataFrame,
                   ...
                   next_expiry_chain: Optional[pd.DataFrame] = None,
                   pr: dict = PRIOR) -> dict:
    """Full §3.4 emission. Every number computed from the supplied chains."""
    # ---- DTE validity (D-MA-06) ----
    if dte_days < pr["dte_splice_days"]:
        if next_expiry_chain is None:
            return {"status": "EXPIRY_DEGENERATE",
                    "detail": f"DTE={dte_days:.2f} < {pr['dte_splice_days']}; "
                              "no next-expiry chain supplied — series gaps.",
                    "provenance": "PRIOR:dte_splice_days"}
        curr_chain = next_expiry_chain            # splice; caller stamps expiry_measured
