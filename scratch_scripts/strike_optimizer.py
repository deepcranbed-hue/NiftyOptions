...
def _metrics(grid, dens, strat: Strat, cost_per_leg):
    ...
    ev = float(np.trapezoid(payoff * dens, grid))
    win = payoff > 0
    pop = float(np.trapezoid(dens[win], grid[win])) if win.any() else 0.0
...
def optimize(chain, rnd, *, objective="ev", weights=None, max_loss_budget_pts=None,
    ...
    if bias is not None and abs(bias) > 1e-9:
        mean = float(np.trapezoid(grid * market_dens, grid))
        sd = float(np.sqrt(np.trapezoid((grid - mean) ** 2 * market_dens, grid)))
    ...
    return {
        ...
        "caveats": [
            ...
            + ("INCLUDED — max_loss is a grid floor, true loss is unbounded."
...
if __name__ == "__main__":
    ...
    grid = np.linspace(23000,25100,400); dens = np.exp(-0.5*((grid-spot)/290)**2); dens/=np.trapezoid(dens,grid)

    chain = {"strikes":strikes,"call_ltp":call_ltp,"put_ltp":put_ltp,"spot":spot,
             "call_oi":call_oi,"put_oi":put_oi,"call_oi_chg":call_oi_chg,
             "put_oi_chg":put_oi_chg,"lot_size":65}
