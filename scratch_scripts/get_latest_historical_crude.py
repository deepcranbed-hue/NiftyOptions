...
def get_latest_price(instrument_key):
    # Query for last 3 days to catch the latest active candle
    url = f"https://api.upstox.com/v2/historical-candle/{instrument_key}/1minute/2026-07-27/2026-07-25"
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            return {"status": "error", "detail": "No candles returned"}
        return {"status": "error", "detail": f"HTTP {res.status_code}: {res.text}"}
    except Exception as e:
        return {"status": "error", "detail": str(e)}

if __name__ == "__main__":
    print("=== LATEST CRUDE VALUES (JULY 25 - JULY 27, 2026) ===")
