...
def sync_all_symbols():
    ...
    try:
        ...
        # ---------------------------------------------------------------
        # Daily EQUITY bars: Yahoo, via daily_bars.py — NOT Breeze.
        #
        # Breeze serves only ~1 year of daily history, and wrote its bars under a
        # different ts string ('...T00:00:00Z') than the Yahoo rows already in the
        # table ('...T00:00:00'). Since ts is part of the primary key, those are
        # two keys for the SAME session: the feeds never overwrote each other, they
        # accumulated. That is why TATAMOTORS and ZOMATO were stuck at 246 bars and
        # why NIFTY carries 22 duplicated sessions.
        #
        # Breeze is still used above for 1-minute bars, and below for FUTURES daily
        # bars, which Yahoo does not carry. Only the equity daily path moved.
        # ---------------------------------------------------------------
        print("Syncing 1-day historical data from Yahoo (full history)...")
        from daily_bars import fetch_best, write_daily, LISTED_FROM
        daily_floor = datetime(2018, 1, 1)
        for symbol in NIFTY_50_SYMBOLS:
            floor = daily_floor
            if symbol.upper() in LISTED_FROM:
                listed = datetime.strptime(LISTED_FROM[symbol.upper()], "%Y-%m-%d")
                if listed > floor:
                    floor = listed

            # Incremental: re-fetch from a few sessions before the watermark so
            # late corrections are picked up. No watermark -> full history, which
            # is the case the old 365-day floor could never recover from.
            ...
            if watermark:
                try:
                    wm = datetime.strptime(watermark[:10], "%Y-%m-%d") - timedelta(days=5)
                    start_dt = max(floor, wm)
                except Exception:
                    start_dt = floor

            end_dt = now_ist.replace(tzinfo=None) + timedelta(days=1)
            if start_dt >= end_dt:
                continue
            try:
                rows, ticker = fetch_best(symbol, start_dt, end_dt, log=lambda *_: None)
                if rows:
                    write_daily(rows, symbol, DB_PATH, exchange="NSE")
                else:
                    sys.stderr.write(f"No Yahoo daily bars for {symbol}\n")
            except Exception as err:
                sys.stderr.write(f"Failed daily bars for {symbol}: {err}\n")
            time.sleep(0.05)
