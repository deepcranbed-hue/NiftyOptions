...
def main():
    ...
    for symbol, yf_ticker in SECTORS.items():
        print(f"Fetching {symbol} from yfinance ({yf_ticker})...")
        
        # We explicitly request auto_adjust=True to fix splits/bonuses
        # and avoid unadjusted phantom crashes.
        df = yf.download(yf_ticker, start=start_date, auto_adjust=True, progress=False)
        ...
        for index, row in df.iterrows():
            ...
            c.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, ("NSE", symbol, "1d", ts_str, open_p, high_p, low_p, close_p, vol))
            inserted += 1
            total_inserted += 1
            
        conn.commit()
        print(f"  -> Upserted {inserted} clean split-adjusted bars for {symbol}")
