#!/usr/bin/env python3
"""sync_finnifty_bars_yf — daily bars for FINNIFTY constituents.
...
def main():
    ...
    print(f"database: {db}\nFINNIFTY constituents ({len(FINNIFTY)}), {'FULL' if full else 'incremental'}:")
    res = sync_symbols(FINNIFTY, db, full=full)
