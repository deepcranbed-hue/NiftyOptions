...
--------------------------------------------------------------
    ...
    ./data_agent/breeze_env/bin/python data_agent/fetching/backfill_daily_bars.py --verify-only

    # fix the two broken symbols (--replace drops their Breeze rows first)
    ./data_agent/breeze_env/bin/python data_agent/fetching/backfill_daily_bars.py \
...
_ROOT = os.path.dirname(os.path.dirname(_HERE))     # data_agent/fetching -> repo root
sys.path.insert(0, _HERE)                            # daily_bars sits beside this file
sys.path.append(_ROOT)

# DRY (CLAUDE.md): fetching, writing and verifying daily bars lives in exactly one
# place, shared with sync_nifty50_bars_yf.py. Imported AFTER the
